...
타임아웃이 찍히니 `RestClientConfig`의 read timeout 4초가 짧은 줄 알았습니다. 늘리기 전에 실제 응답 시간을 재봤습니다.
...
실패가 5초 아래로 내려오지 않습니다. 느려지다 실패하는 것이 아니라, 게이트웨이가 정상이면 즉시 응답하고 아니면 자기 타임아웃까지 붙잡고 있다가 504를 던집니다.

**그래서 타임아웃을 늘리는 것은 해결이 아니었습니다.** 성공은 이미 4초 안에 들어오므로 늘려서 새로 건지는 요청이 없고, 4초짜리 실패가 10초짜리 실패로 바뀔 뿐입니다. read timeout이 4초라 504를 받기 전에 우리가 먼저 끊고, 그래서 화면에는 타임아웃으로 보였습니다.

대신 곧바로 다시 호출하면 상당수가 성공했습니다. 4초 컷으로 15회 재현했을 때 성공률이 40%에서 80%로 올라갔습니다.

**해결**

- 재시도 1회 (`ExternalCallGuard`). 지연이 아니라 상태가 갈리는 실패이므로 백오프를 두지 않습니다
...
```text
1) 첫 요청       4,604ms   재시도 후 쿨다운 진입
2) 바로 다음         66ms   외부 호출을 건너뛰고 저장된 값 사용
3) 60초 뒤       4,075ms   쿨다운 해제, 다시 호출
```

쿨다운 중이 오히려 빠릅니다. 기다리지 않고 폴백으로 가기 때문입니다.
