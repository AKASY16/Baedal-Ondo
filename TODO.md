...
- [x] **공개 진입 구조·SEO·게스트 지역 선택** — `/`의 로그인 리다이렉트를 공개 랜딩페이지로 바꾸고
  ...
  canonical·Open Graph·WebSite JSON-LD·네이버 소유확인 메타와 `robots.txt`·`sitemap.xml`을 정리했다.
  절대 URL favicon과 한국어 응답 언어를 지정하고, 로그인·회원가입·동적 게스트 대시보드는 `noindex`로 두어
  sitemap에서 제외했다. 게스트 대시보드는 기존 무작위 지역을
  최초 기본값으로 유지하면서 서울 25개 구를 직접 선택해 `regionId` 기준으로 다시 조회할 수 있다.
  랜딩의 `02 · START`도 `01 · SIGNALS`와 같은 제목 그리드로 맞추고, 첫 화면 설명은 데이터 근거와
  **0~100점 지표 의미가 별도 줄에서 읽히도록** 명시적으로 분리했다. 공개·보호 경로 어디에도 속하지 않는
  URL은 로그인 대신 404를 주되 기능 경로군의 로그인 보호는 유지했다. 공개 URL·Security·SEO·지역 선택과
